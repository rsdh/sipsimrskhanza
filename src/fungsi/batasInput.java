...
public class batasInput {
    ...
    //RSDH digit with maximum
    public PlainDocument getOnlyAngkaMax(final JTextField inputan, final double max) {
        filter = new PlainDocument(){
            @Override
            public void insertString(int offs, String str, AttributeSet a)throws BadLocationException{
                StringBuffer buf = new StringBuffer();
                ...
                int x = inputan.getText().length();
                if(x < length){
                    double chk = Double.parseDouble(inputan.getText() + new String(buf));
                    if (chk <= max)
                        super.insertString(offs, new String(buf), a);
                }
            }
